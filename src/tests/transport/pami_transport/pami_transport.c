...
void optiq_execute_jobs(struct optiq_pami_transport *pami_transport)
{
	...
	if (pami_transport->extra.isDest && pami_transport->extra.expecting_length == 0) {
            for (int i = 0; i < pami_transport->size; i++) {
...
{   
    ...
    pami_transport->extra.sent_bytes += rput_cookie->message_header->length;
}
...
void optiq_recv_rput_done_notification_fn(pami_context_t context, void *cookie, const void *header, size_t header_size, const void *data, size_t data_size, pami_endpoint_t origin, pami_recv_t *recv)
{
	...
	pami_transport->extra.recv_bytes[message_header->source] += message_header->length;
    } else {
...
void optiq_recv_mr_destination_request_fn (pami_context_t context, void *cookie, const void *header, size_t header_size, const void *data, size_t data_size, pami_endpoint_t origin, pami_recv_t *recv)
{
    ...
    int source = (*(int *)data);
    int offset = pami_transport->extra.rdispls[source];

    pami_transport->extra.recv_mr->header_id = *((int*)header);
    pami_transport->extra.recv_mr->offset = offset;

    optiq_pami_send_immediate (pami_transport->context, MR_RESPONSE, NULL, 0, pami_transport->extra.recv_mr, sizeof(struct optiq_memregion), pami_transport->endpoints[origin]);
