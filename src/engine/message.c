...
struct optiq_message* get_messages(int num_messages)
{
    ...
    for (int i = 0; i < num_messages; i++) {
        ...
        messages[i].current_offset = 0;
        messages[i].service_level = 0;
        messages[i].next_dest = 0;
        messages[i].header.final_dest = -1;
        messages[i].header.flow_id = -1;
        messages[i].header.original_length = 0;
        messages[i].header.original_offset = 0;
    }
...
struct optiq_message* get_messages_with_buffer(int num_messages, int buffer_size)
{
    ...
    for (int i = 0; i < num_messages; i++) {
        messages[i].length = 0;
        messages[i].buffer = (char *)core_memory_alloc(buffer_size, "messages[i]->buffer", "get_messages_with_buffer");
        messages[i].current_offset = 0;
        messages[i].service_level = 0;
        messages[i].next_dest = 0;
        messages[i].header.final_dest = -1;
