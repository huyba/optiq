...
void optiq_topology_compute_routing_order(struct topology *self, int *order)
{
    int num_dims = self->num_dims;
    int *size = self->size;
...
int optiq_topology_compute_neighbors_bgq(struct topology *self, int *coord, int *neighbors) 
{
    int num_neighbors = 0;
    int nid = 0;
    int *size = self->size;
