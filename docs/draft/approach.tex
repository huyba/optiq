...
The input to the algorithm include the set of source-destination pairs $\mathcal{P}$ = \{(S, D) $|$ S, D $\in~N$\}, their $k$ shortest paths {$k\_paths$}, the maximum allowed load on a link $maxload$ and the link load matrix $load$. $load$ is a table of loads on all physical links. Whenever a link $l=(u,v)$ is selected by the heuristic, the corresponding entry $load(l) = load[u][v]$ is increased by 1. The heuristic outputs a set of selected paths $s\_paths$ for data movement for each source-destination pair. 

We iterate through all pairs of source-destination in $\mathcal{P}$. For each source-destination pair, we select a path $p$ from $k\_paths$ (lines 2--4). The selected path is removed from the set $k\_paths_i$ (line 5), where $i$ is the index of the selected source-destination pair. A path from the source node to the destination node is a set of links through intermediate nodes. We call this set of physical links $links$ (line 6). We check if adding $p$ to $s\_paths$ would satisfy the load condition for links in $p$ (lines 7--9). If load $load(l)$ on all links $l \in links$ is below $maxload$, the path $p$ is added to $s\_paths_i$ (line 8). Also, the $load$ table is updated for all links $l \in links$.


We check the load by going through all links used by the path and examine their current loads in the $load$ table in comparison with $maxload$. If their current load adding by 1 is not over $maxload$ we then add the path $p$ into $s\_paths$, update the $load$ table and remove the path from $k\_paths$. We iterate through all pairs and add at most one path per pair at a time. The algorithm completes when we either running out of paths to add or the maximum load of physical links in $load$ table is over $maxload$.
