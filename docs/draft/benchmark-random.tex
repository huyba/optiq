...
\begin{table*}%[!htbp]
   \centering
    ...
    \end{tabular}
    \caption{Throughput (GB/s) for Optimization (OPT), Heuristic (HEU) and MPI\_Alltoallv (MPI) for 5 different random pairings between sources and destinations in 1024-node partition.}
...
\begin{table*}[!htbp]
   \centering
    ...
    \end{tabular}
    \caption{Throughput (GB/s) for MPI\_Alltoallv (MPI), Optimization (Opt), Heuristic (Heu) and average number of hops (Dist) between sources and destinations for 5 different random pairings (Config) of sources and destinations in 512-node partition.}
